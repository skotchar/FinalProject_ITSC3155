...
def visualization_layout(logos, figures, dissable_button=False):
    return html.Button('?', id='visualization_tutorial_button', n_clicks=0)
...
@app.callback(
    ...
    dd.Input('home_tutorial_button', 'n_clicks'),
    dd.Input('visualization_tutorial_button', 'n_clicks'),
    dd.Input('stop_tutorial', 'n_clicks'), suppress_callback_exceptions=True)
def switch_to_tutorial_mode(value):
    ...
    if value == 0:
        raise PreventUpdate
    elif input_id == 'stop_tutorial':
        logos = logo
        arrows = arrow
        figures = fig
        dissable_button = False
        value = 0
        return switch_layout(2, logos, arrows, figures, dissable_button)
    elif input_id == 'home_tutorial_button' or input_id == 'visualization_tutorial_button':
        logos = logo
        arrows = arrow
        figures = fig
